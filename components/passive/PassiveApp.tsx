                  ...
                  return (
                    <li key={position.id} className="card" style={{ padding: '0.7rem' }}>
                      ...
                      <p className="muted" style={{ marginTop: '0.2rem' }}>
                        Avg: {position.avg_buy_price.toLocaleString()} gp | Current: {Math.round(position.last_price ?? 0).toLocaleString()} gp
                      </p>
                      <div className="row-between" style={{ marginTop: '0.2rem' }}>
                        <span style={{ color: (position.unrealized_profit ?? 0) >= 0 ? 'var(--accent-2)' : 'var(--danger)' }}>
                          {(position.unrealized_profit ?? 0) >= 0 ? '+' : ''}{Math.round(position.unrealized_profit ?? 0).toLocaleString()} gp
                        </span>
                        <span style={{ 
                          color: roi >= 0 ? 'var(--accent-2)' : 'var(--danger)',
                          fontWeight: 700,
                          fontSize: '0.85rem'
                        }}>
                          {roi >= 0 ? '+' : ''}{roi.toFixed(1)}%
