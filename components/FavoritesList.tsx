              ...
              >
                ...
                <div className="p-3 bg-slate-900/30 border-t border-slate-700 space-y-2">
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      const currentPrice = price ? Math.round((price.high + price.low) / 2) : 'unknown';
                      openChat(`Tell me about ${favorite.name}. Current price is ${currentPrice}gp. Is this a good flip opportunity?`);
                    }}
                    className="w-full py-2 px-3 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded transition-colors text-sm flex items-center justify-center gap-2"
                  >
